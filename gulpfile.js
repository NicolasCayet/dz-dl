...
gulp.task('copy:assets', function(cb) {
    gulp.src([
        ...
        'client/.htaccess',
        'client/assets/**',
        'client/deezer-channel-jsonp.html',
        '!client/app/**/*.ts'], {base: "client"})
        .pipe(gulp.dest('dist'))
        .on('end', cb);
});
...
gulp.task('compile', function (cb) {
    ...
    var configJson = fs.readFileSync('client/config/local.json');
...
gulp.task('watch', function() {
    gulp.watch(['client/index.html', "client/app/**/*.{html,htm,css,js}"], function() {
